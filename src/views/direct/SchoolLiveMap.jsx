...
const getMarkerLabelText = (entry, mode = "vehicle") => {
    ...
    switch (mode) {
        ...
        case "route": {
            console.log("Route Label Entry:", entry);
...
const MapComponent = ({
    ...
    */
    ...
    const getMarkerLabel = (entry, mode,showRouteInfo) => {
        if (showRouteInfo) {
    return (
      entry.route_name ||
      entry.assigned_route ||
...
<path fill="${safeColor}" d="M12 2c4.418 0 8 3.134 8 7 0 5.25-8 13-8 13S4 14.25 4 9c0-3.866 3.582-7 8-7Zm0 4a3 3 0 1 0 .001 6.001A3 3 0 0 0 12 6Z"/>
   ...
   const clusterStyleFunction = (feature) => {
    ...
    if (size === 1) {
        // Extract style directly from raw wrapped child element instead of grouping it
...
useEffect(() => {
    ...
    // Target inner raw unclustered layer source to bypass generic styling maps
    const vectorSource = vectorLayer.getSource().getSource();
    const allMarkers = [...gpsData, ...policeData];
    const currentImeis = new Set();
    ...
    allMarkers.forEach((entry) => {
        // FORCE STRINGS: Normalize unique identifiers to strings to avoid type-check failures
        const rawImei = entry.imei || entry.id || entry.vehicle_registration_number;
        if (rawImei === undefined || rawImei === null) return;
        ...
        const vehicleType = entry?.vehicle_type || entry?.device_tag_info?.category_info?.category || "school bus";

        let feature = activeFeaturesRef.current[imei];

        if (!feature) {
            // Instantiate marker feature if it is completely missing from cache
            feature = new Feature({
            ...
            // Console warning check to verify asset generation tracks accurately
            const resolvedStyle = getIconStyle(entry, vehicleType, markerLabelMode, allMode);
            if (!resolvedStyle) {
                console.error(`[LIVE TRACKING] Icon path failed to compute for asset string style: ${vehicleType}`);
            }
            
            feature.setStyle(resolvedStyle);
            activeFeaturesRef.current[imei] = feature;
            vectorSource.addFeature(feature);
        } else {
            // Animate transition between live GPS coordinates smoothly
            const startCoords = feature.getGeometry().getCoordinates();
            const startTime = performance.now();

            feature.set("entryData", entry);
            ...
            const frame = (now) => {
                ...
                // Safe comparative lookups using string bounds
                if (focusEntry && (String(focusEntry.imei) === imei || String(focusEntry.id) === imei || String(focusEntry.vehicle_registration_number) === imei)) {
            ...
            if (Math.abs(startCoords[0] - targetCoords[0]) > 0.000001 || Math.abs(startCoords[1] - targetCoords[1]) > 0.000001) {
                ...
                feature.getGeometry().setCoordinates(targetCoords);
                feature.setStyle(getIconStyle(entry, vehicleType, markerLabelMode, allMode));
            }
        }
    });

    // Clean up out-of-bounds vehicles cleanly
    Object.keys(activeFeaturesRef.current).forEach(imeiKey => {
        if (!currentImeis.has(String(imeiKey))) {
            const obsoleteFeature = activeFeaturesRef.current[imeiKey];
            if (obsoleteFeature.get("animFrameId")) {
                cancelAnimationFrame(obsoleteFeature.get("animFrameId"));
            }
            vectorSource.removeFeature(obsoleteFeature);
            delete activeFeaturesRef.current[imeiKey];
        }
    });
