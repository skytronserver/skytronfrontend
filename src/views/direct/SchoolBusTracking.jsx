...
import { getUseOldGeocodingApi } from "../../services/HomePage";
import MapComponent from "./SchoolLiveMap";
...
const SchoolBusTracking = () => {
  ...
  const getSchoolBusDisplayValue = (
    row,
    key
  ) => {
    ...
    switch (key) {
      ...
      case "speed":
        return `${row.speed || 0} km/h`;

      case "gps_status":
        return row.gps_status === "1"
          ? "Online"
          : "Offline";

      case "ignition_status":
        return row.ignition_status === "1"
          ? "ON"
          : "OFF";

      case "main_power_status":
        return row.main_power_status === "1"
          ? "ON"
          : "OFF";

      case "location_available":
        return row.location_available
          ? "YES"
          : "NO";
      ...
      case "route_stops":
        ...
        if (
          !row.route_stops ||
          row.route_stops.length === 0
        ) {
          return "No Stops";
        }

        return row.route_stops
          .map(
            (stop, index) =>
              `${index + 1}. ${stop.name}
             (${stop.timing})`
  ...
  const computeRow = (processedItem) => {
    ...
    const vehicleType =
      processedItem?.vehicle_type ||
      processedItem?.device_tag_info?.category_info?.category ||
      "bus";
  ...
  const retriveMapData = async () => {
    try {
      ...
      const response =
        await HomePageService.getSchoolBusOverview();

      const buses =
        response?.data?.data?.buses || [];
        console.log("School Bus API Response:", buses);

buses.forEach((bus) => {
  console.log(
    "Vehicle:",
    bus?.bus?.vehicle_reg_no,
    "Route:",
    bus?.assigned_route
  );
});

      const formattedData = buses
        .map((item) => {
          const location = item.location || {};
          console.log("Mapped Route Data:", {
  vehicle: item?.bus?.vehicle_reg_no,
  routeId: item?.assigned_route?.id,
  routeName: item?.assigned_route?.name,
  stops: item?.assigned_route?.stops,
});
          ...
          return {
            ...
            vehicle_registration_number:
              item?.bus?.vehicle_reg_no || "N/A",

            vehicle_make:
              item?.bus?.vehicle_make || "",

            vehicle_model:
              item?.bus?.vehicle_model || "",
            ...
            assigned_route:
              item?.assigned_route?.name || "No Route",

            route_id:
              item?.assigned_route?.id || "",

            route_stops:
              item?.assigned_route?.stops || [],
            ...
            gps_status:
              String(location?.gps_status || "0"),

            ignition_status:
              String(location?.ignition_status || "0"),

            main_power_status:
              String(location?.main_power_status || "0"),
      ...
      setTableDataTop(formattedData);

      setFilteredData([]);

      fullRawRef.current = formattedData;

      fullDataRef.current = formattedData;

      setVisibleCount(formattedData.length);

      if (formattedData.length === 1) {
        setSelectedId(
          `vehicle-${formattedData[0].imei}`
        );

        setFocusedEntry(formattedData[0]);
      }
  ...
  useEffect(() => {
  console.log("Marker Label Mode:", markerLabelMode);
}, [markerLabelMode]);

  const handleButtonClick = (id) => {
    const selectedRow = tableDataTop.find(
      (row) => `vehicle-${row.imei}` === id
    );
  ...
  const getIconStyle = (data) => {
    ...
    const vehicleType =
      data?.vehicle_type ||
      data?.device_tag_info?.category_info?.category ||
      "school bus";
  ...
  setTableDataTop(result);

  if (result.length === 1) {
    setFocusedEntry(result[0]);
    setSelectedId(`vehicle-${result[0].imei}`);
  }
};
  ...
  control={ 
    <Switch
      ...
      onChange={(event) => {
        setShowRouteInfo(event.target.checked);
      ...
      {selectedId && (
        <TableContainer component={Paper} className="skytron-table-container" sx={{ mt: 2, maxHeight: '400px' }}>
          <Table stickyHeader className="skytron-table">
            <TableHead>
              <TableRow>
                {Object.keys(schoolBusKeyMapping).map((key) => (
                  <TableCell
                    key={key}
                    className="skytron-table-header-cell"
                    sx={{ backgroundColor: '#f5f5f5', fontWeight: 'bold' }}
                  >
                    {schoolBusKeyMapping[key]}
                  </TableCell>
                ))}
              </TableRow>
            </TableHead>
            <TableBody>
              {filteredData.length > 0 ? (
                filteredData.map((row, index) => (
                  ...
                  >
                    {Object.keys(schoolBusKeyMapping).map((key) => (
                      ...
                      >
                        {getSchoolBusDisplayValue(row, key)}
                      </TableCell>
                    ))}
                  </TableRow>
                ))
                ...
                <TableRow key="no-filtered-data">
                  <TableCell
                    colSpan={Object.keys(schoolBusKeyMapping).length}
