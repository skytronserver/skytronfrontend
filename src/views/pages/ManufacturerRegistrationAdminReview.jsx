...
const MANUFACTURER_ROLES = [
  ...
  "AIS-140 Device Manufacturer",
];

const ManufacturerRegistrationAdminReview = () => {
  ...
  const requests = useMemo(() => {
    void refreshKey;
    return PublicRegistrationMockService.listRequests().filter((r) =>
      MANUFACTURER_ROLES.includes((r.role || "").trim())
    );
  }, [refreshKey]);
      ...
      <Grid container spacing={3} justifyContent="center">
        <Grid item xs={12} md={10} lg={9}>
          <Paper sx={paperStyle}>
            <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ mb: 2 }}>
              <Box sx={{ flex: 1 }}>
                <Typography variant="h5" sx={{ fontWeight: "bold", color: "#800080" }}>
                  Manufacturer Registration Requests
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Approve or reject requests for Vehicle Manufacturer and AIS-140 Device Manufacturer.
                </Typography>
              </Box>
            </Stack>
            ...
            {requests.length === 0 ? (
              <Alert severity="info">No manufacturer registration requests found.</Alert>
            ) : (
              <Stack spacing={2}>
                {requests.map((req) => (
...
export default ManufacturerRegistrationAdminReview;
